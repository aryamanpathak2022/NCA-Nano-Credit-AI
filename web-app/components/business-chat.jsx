...
export function BusinessChat() {
    ...
    const getAIResponse = async () => {
      if (messages.length > 0 && messages[messages.length - 1].sender === 'user') {
        try {
          ...
          setMessages(
            prev => [...prev, { text: 'Sorry, an error occurred while processing your request.', sender: 'ai' }]
          )
        }
      }
    }
  ...
  return (
    (<div className="flex flex-col h-screen bg-slate-950 relative overflow-hidden">
      <div
        className="absolute top-0 left-0 w-96 h-96 bg-purple-500 rounded-full opacity-10 -translate-x-1/2 -translate-y-1/2" />
      <div
        className="absolute bottom-0 right-0 w-96 h-96 bg-purple-500 rounded-full opacity-10 translate-x-1/2 translate-y-1/2" />
      <div
        className="flex items-center justify-between p-4 border-b border-slate-800 relative z-10">
